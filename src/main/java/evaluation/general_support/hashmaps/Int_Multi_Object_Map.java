...
/**
 * This map provides a hash-map implementation for mapping primitive integer keys to multiple objects
 * of a given type {@code T}.
 * We assume keys are non-negative to keep the implementation simple and efficient.
 */
public class Int_Multi_Object_Map<T> {
    ...
    /**
     * Creates an empty {@link Int_Multi_Object_Map} instance.
     */
    public Int_Multi_Object_Map() {
     ...
     */
    public Int_Multi_Object_Map(int capacity) {
        if (capacity % 2 != 0)
            throw new IllegalArgumentException("Int_Multi_Object_Map requires its capacity to be a power of 2");
    ...
    /**
     * Method to associate a given value to a key. Simply associates an additional value to a key if
     * a key already has a value associated to it.
     ...
     */
    public void add(int key, long preHash, T value) {
        checkKeyNonNegative(key);

        // Check if the map already contains the key
        int index = find(key, preHash);
        if (index != -1) {
            // If so, simply add the new value to this key's list of values
    ...
    public List<T> get(int key, long preHash) throws NoSuchElementException {
        ...
        throw new NoSuchElementException("Int_Multi_Object_Map does not contain key " + key);
    }
    ...
    private void growArrays() {
        ...
        if (newSize > Integer.MAX_VALUE - 1)
            throw new UnsupportedOperationException("Int_Multi_Object_Map has grown too large");
        ...
        // No need to grow the values arraylist as it is a dynamic size structure already
    ...
    private void checkKeyNonNegative(int key) throws IllegalArgumentException {
        if (key < 0)
            throw new IllegalArgumentException("Int_Multi_Object_Map does not support negative keys");
    }

    /**
     * Method to obtain a primitive-type specific iterator to iterate over the keys of this map.
     * @return A new {@link Int_Multi_Object_Map_Iterator} for this map.
     */
    public Int_Multi_Object_Map_Iterator getIterator() {
        return new Int_Multi_Object_Map_Iterator();
    }

    /**
     * Class definition for iterating over the keys of a {@link Int_Multi_Object_Map}.
     */
    public class Int_Multi_Object_Map_Iterator {
        ...
        /**
         * Creates a new {@link Int_Multi_Object_Map_Iterator} instance.
         * @return The new {@link Int_Multi_Object_Map_Iterator} instance.
         */
        private Int_Multi_Object_Map_Iterator() {
            this.currentIndex = 0;
        ...
        public int next() throws NoSuchElementException {
            if (!this.hasNext())
                throw new NoSuchElementException("Int_Multi_Object_Map_Iterator has reached the end of the iteration");
