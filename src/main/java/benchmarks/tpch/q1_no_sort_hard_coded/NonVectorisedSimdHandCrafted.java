...
public class NonVectorisedSimdHandCrafted {
     ...
     */
    private VectorSpecies<Integer> VECTOR_SPECIES_INT = jdk.incubator.vector.IntVector.SPECIES_PREFERRED;
    ...
    public void executeQuery(Blackhole bh) throws IOException {
        boolean[] SIMD_sel_mask = new boolean[VECTOR_SPECIES_INT.length()];
        while (lineitem_table.loadNextBatch()) {
            ...
            // Manual Optimisation: perform SIMD-ed filtering
            MemorySegment col_6_ms = MemorySegment.ofAddress(lineitem_vc_6.getDataBufferAddress(), recordCount * lineitem_vc_6.TYPE_WIDTH);
            for (int currentIndex = 0; currentIndex < recordCount; currentIndex += VECTOR_SPECIES_INT.length()) {
                jdk.incubator.vector.VectorMask<Integer> inRangeSIMDMask = VECTOR_SPECIES_INT.indexInRange(currentIndex, recordCount);
                jdk.incubator.vector.IntVector SIMDVector = jdk.incubator.vector.IntVector.fromMemorySegment(VECTOR_SPECIES_INT, col_6_ms, currentIndex * lineitem_vc_6.TYPE_WIDTH, java.nio.ByteOrder.LITTLE_ENDIAN, inRangeSIMDMask);
                jdk.incubator.vector.VectorMask<Integer> SIMDVector_sel_mask = SIMDVector.compare(VectorOperators.LE, 10471, inRangeSIMDMask);
                SIMDVector_sel_mask.intoArray(SIMD_sel_mask, 0);

                // ADAPTED ORIGINAL CODE
                for (int subIndex = 0; subIndex < VECTOR_SPECIES_INT.length(); subIndex++) {
                    if (!SIMD_sel_mask[subIndex])
                        continue;

                    int aviv = currentIndex + subIndex;
                    ...
                    long group_key_pre_hash = Char_Arr_Hash_Function.preHash(ordinal_value_0);  // Hashing has not been SIMD due to little performance reward in other experiments
